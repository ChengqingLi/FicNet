...
def train(epoch, model, loader, optimizer, args=None):
    ...
    for i, ((data, train_labels), (data_aux, train_labels_aux)) in enumerate(zip(tqdm_gen, train_loader_aux), 1):
        ...
        #absolute_loss1 = F.cross_entropy(loss_class, train_labels[k:])
...
def train_main(args):
    ...
    set_seed(args.seed)
    maxtestacc,maxepoch=0.0,0
    # 构建 SummaryWriter
    for epoch in range(1, args.max_epoch + 1):
        start_time = time.time()
        train_loss, train_acc, _ = train(epoch, model, train_loaders, optimizer, args)
        if epoch==args.max_epoch-15:
            args.val_episode=1200
            val_sampler = CategoriesSampler(valset.label, args.val_episode, args.way, args.shot + args.query)
            val_loader = DataLoader(dataset=valset, batch_sampler=val_sampler, num_workers=8, pin_memory=True)
            val_loader = [x for x in val_loader]
        if epoch<=20 or epoch>=(args.max_epoch-15):
            val_loss, val_acc, _ = evaluate(epoch, model, val_loader, args, set='val')
        if epoch==1 or epoch==2 or epoch%10==0 or epoch>=(args.max_epoch-15):
            test_loss, test_acc, _ = evaluate(epoch, model, test_loader, args, set='test')
            if maxtestacc<test_acc:
                maxtestacc=test_acc
                maxepoch=epoch
                torch.save(dict(params=model.state_dict(), epoch=epoch), os.path.join(args.save_path, 'testmax_acc{}.pth'.format(maxepoch)))
                torch.save(optimizer.state_dict(), os.path.join(args.save_path, 'testoptimizer_max_acc.pth'))
        if not args.no_wandb:
        ...
        if epoch==args.max_epoch:
            torch.save(dict(params=model.state_dict(), epoch=epoch), os.path.join(args.save_path, 'last_epoch.pth'))
        if val_acc > max_acc:
            print('[ log ] *********A better model is found %.3f in %d th epoch*********'% (val_acc,max_epoch))
        ...
        lr_scheduler.step()
    print('[ log ] *********A better model is found %.3f in %d th epoch*********'% (max_acc,max_epoch))
    print('best performance is %.3f in %d th epoch'% (maxtestacc,maxepoch))
    return model
